                ...
                <el-card shadow="hover" class="login-form-card">
                    ...
                    <el-form ref="registerFormRef" :model="registerForm" :rules="registerRules" class="login-form"
                        ...
                        <div class="avatar-upload-container">
                             <el-form-item prop="userAvatarUrl">
                                <el-upload
                                ...
                                >
                                    <img v-if="registerForm.userAvatarUrl" :src="registerForm.userAvatarUrl" class="avatar" />
                                    <el-icon v-else class="avatar-uploader-icon"><Plus /></el-icon>
                                    <div class="upload-tip" v-if="!registerForm.userAvatarUrl">点击上传头像</div>
                                </el-upload>
                             </el-form-item>
                        </div>
...
const customUpload = async (options) => {
    try {
        const response = await uploadUser(options.file)
        // 根据你的描述，从 response.data 可以获取图像的 url
        // 假设后端返回结构是 { code: 200, data: 'http://...', msg: '...' }
        // 如果 response.data 本身就是 url 字符串，请直接使用 response.data
        
        if (response.code === 200) {
             registerForm.userAvatarUrl = response.data
             ElMessage.success('头像上传成功')
        } else {
             // 如果接口直接返回的是data里的内容，可能需要调整这里。
             // 这里假设标准的 {code, data, msg} 结构
