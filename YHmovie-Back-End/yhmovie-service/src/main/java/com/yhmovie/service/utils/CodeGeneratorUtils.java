...
public class CodeGeneratorUtils {
    public static void main(String[] args) {
        FastAutoGenerator.create("jdbc:mysql://localhost:3307/yh_movie", "root", "123456")
                .globalConfig(builder -> {
                    builder.author("h3o7") // 设置作者
                            .outputDir(System.getProperty("user.dir") + "/src/main/java/com/yhmovie"); // 指定输出目录（绝对路径）
                })
                .packageConfig(builder -> {
                    builder.parent("service") // 设置父包名（生成到 src/main/java/blogsystem 下）
